...
random.seed(42) # Every run of split_kather will give the same train/val/test split.import os
import os.path as osp
import random
import shutil

random.seed(42)  # Every run gives the same train/val/test split.
...
# =============================================================================
# Split Kather16 Dataset into Train/Val/Test
# =============================================================================
def split_kather16(path_data_in):
    # Save processed dataset to a writable location
    ...
    if any(os.listdir(p) for p in [path_train, path_val, path_test]):
        print("Train/val/test folders already contain files. Skipping split.")
        return
    ...
    for f in subfolders:
        ...
        val_len = int(0.15 * len(im_list))
        test_len = int(0.15 * len(im_list))
        ...
        for im in im_train:
            copy_unique(
                osp.join(path_data_in, f, im),
                osp.join(path_train, f)
            )
        ...
        for im in im_test:
            copy_unique(
                osp.join(path_data_in, f, im),
                osp.join(path_test, f)
            )

    print(f"Kather16 split complete.\nSaved to: {base_out}")


# =============================================================================
# Split Kather19 Dataset into Train/Val/Test
# =============================================================================
def split_kather19(path_data_in):
    ...
    for f in subfolders:
        ...
        train_len = int(0.70 * len(im_list))
        val_len = int(0.15 * len(im_list))
        test_len = int(0.15 * len(im_list))
    ...
    print(f"Kather19 split complete.\nSaved to: {base_out}")
# def safe_makedir(path):
#     os.makedirs(path, exist_ok=True)
